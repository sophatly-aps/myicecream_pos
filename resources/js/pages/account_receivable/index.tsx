...
export default function AccountReceivable({ orders, filters, settings, totalDue }: any) {

    const [isAlertOpen, setIsAlertOpen] = useState(false);
    const [alertAction, setAlertAction] = useState<"trash" | "restore" | "force_delete" | null>(null);
        ...
        axios.put(`/sales/${editOrder.id}`, editFormData)
            .then(res => {
                if (res.data.success) {
                    ...
                    setEditOrder(null);
                    router.reload({ only: ['orders', 'totalDue'] });
                }
            })
            .catch(err => {
    ...
    const handleAlertConfirm = () => {
        ...
        if (alertAction === "force_delete") {
            url = `/sales/${selectedOrder.id}/force-delete`;
            router.delete(url);
        }
    ...
    const columns = useMemo(() => [
        ...
        columnHelper.accessor('payment_method', {
            ...
            cell: info => {
                ...
                return (
                    <Badge variant="outline" className="uppercase">{t(`ar.payment_method.${method}`)}</Badge>
                )
            }
        }),
        columnHelper.accessor('payment_status', {
            ...
            cell: info => {
                ...
                return <Badge className={`uppercase text-[10px] ${bgColor}`}>{t(`ar.payment_status.${status}`)}</Badge>;
            },
        }),
        columnHelper.display({
            id: 'actions',
        ...
        <>
            ...
            <div className="flex flex-col gap-4 p-6 w-full overflow-hidden">
                <div className="flex flex-col md:flex-row gap-4 items-end bg-white p-4 rounded-xl border border-gray-200">
                    <div className="flex flex-col gap-1 w-full md:w-48">
                        <label className="text-xs font-bold text-gray-500 uppercase">{t('ar.search')}</label>
                        <Input
                    ...
                    <div className="flex flex-col gap-1 w-full md:w-48">
                        <label className="text-xs font-bold text-gray-500 uppercase">{t('ar.date_search')}</label>
                        <select
                ...
                <div className="bg-red-50 border border-red-100 rounded-xl p-4 flex justify-between items-center shadow-sm">
                    <div>
                        <p className="text-xs font-bold text-red-800 uppercase tracking-widest">{t('ar.total_due')}</p>
                        <p className="text-[10px] text-red-600 mt-1">{t('ar.total_due_description')}</p>
                    </div>
                    <div className="text-2xl font-black text-red-700">
                        {currency}{Number(totalDue || 0).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </div>
                </div>
                        ...
                        <div className="px-8 pb-8 pt-2 bg-[#f1f5f9]">
                            ...
                            <div className="space-y-4 mb-6 min-h-[100px]">
                                {viewOrder.details?.map((detail: any) => (
                                    <div key={detail.id} className="grid grid-cols-12 gap-2 text-[13px]">
                                        ...
                                        <div className="col-span-2 text-center text-gray-800">{detail.quantity}</div>
                            ...
                            <div className="border-t-2 border-gray-200 pt-4 flex justify-end">
                                <div className="w-[60%] space-y-2 text-[13px] text-gray-600">
                                    ...
                                    <div className="flex justify-between border-t-2 border-gray-800 pt-2 mt-2">
                                        <span className="font-bold text-lg text-gray-800">{t('sales.grand_total')}</span>
                            ...
                            <div className="text-right mt-6">
                                <span className="text-[10px] font-bold text-blue-700 tracking-widest uppercase">
                                    {viewOrder.payment_status === 'paid' ? t('sales.payment_status.paid') : viewOrder.payment_status === 'partial' ? t('sales.payment_status.partial') : t('sales.payment_status.due')}
                                </span>
                            </div>
                        </div>
                    )}
            ...
            <Dialog open={!!editOrder} onOpenChange={(open) => !open && setEditOrder(null)}>
                <DialogContent className="max-w-sm rounded-xl">
                    <h2 className="text-lg font-bold mb-4 font-sans">{t('ar.edit_title')}</h2>
                    {editOrder && (
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-2 pt-4">
                                ...
                                >
                                    {isSaving ? t('sales.processing') : t('sales.save_changes')}
                                </Button>
                                <DialogClose asChild>
            ...
            <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            {alertAction === "trash" && "Are you sure you want to move this order to the trash? You can restore it later."}
